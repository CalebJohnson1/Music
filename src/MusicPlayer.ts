...
function playMusic() {
    ...
    audio.play();
    isPlaying = true;
    const playPauseBtn = document.getElementById("playPauseButton") as HTMLButtonElement;
...
function fileSelect() {
    ...
    audioInput.addEventListener('change', (e: Event) => {
        ...
        const musicTracksLayout = document.getElementById("music-tracks") as HTMLDataListElement;
        musicTracksLayout.innerHTML = '';

        for (const file of target.files) {
            const track = document.createElement("li");
            track.className = 'px-2 py-2 hover:bg-slate-800 cursor-pointer border-b border-slate-700 select-none';
            ...
            jsmediatags.read(file, {
                onSuccess: function(tag: any) {
                    ...
                    artist !== '' ? track.textContent = `${artist} - ${title}` : track.textContent = title;
                },
                onError: function(e: Error) {
...
function handleVolumeSlider() {
    ...
    volumeSlider.addEventListener('input', function() {
        ...
        Number(volumeSlider.value) == 0 ? volumeIcon.innerHTML = mutedIcon :
            Number(volumeSlider.value) < 50 ? volumeIcon.innerHTML = volumeLowIcon : volumeIcon.innerHTML = volumeHighIcon;
    });
}
